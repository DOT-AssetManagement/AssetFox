...
namespace RollupSegmentation
{
    public class RollupSegmentation
    {
        ...
        public void DoRollup()
		{
            ...
            RollupMessaging.AddMessage("Begin rollup of network: " + strNetwork + " at " + DateTime.Now.ToString("HH:mm:ss"));
			if( !CompileAreaEquation() )
			{
				RollupMessaging.AddMessage( "Rollup of network aborted: " + strNetwork + " at " + DateTime.Now.ToString( "HH:mm:ss" ) );
                if(apiCall == true)
                {
                    var updateStatus = Builders<RollupModel>.Update
			...
			//Get NetworkID
			RollupMessaging.AddMessage("Getting NETWORKID");
			...
			RollupMessaging.AddMessage("Getting segments from SEGMENT_CONTROL");
			...
			foreach( DataRow row in ds.Tables[0].Rows )
			{
				...
				{
					RollupMessaging.AddMessage( "Warning: Drop table " + strTable + " failed. " + exc.Message );
                }
			}
			bool bPCI = false;
			...
			{
				RollupMessaging.AddMessage( "Warning: Error retrieving PCI count.  PCI not included in rollup." + exc.Message );
				bPCI = true;
            }
            ...
            RollupMessaging.AddMessage("Getting ATTRIBUTE list from ATTRIBUTES table...");
			...
			RollupMessaging.AddMessage("Finished selection of all ATTIRBUTES for all YEARS");
			...
			foreach( String str in m_listAttributes )
			{
				...
				{
					RollupMessaging.AddMessage( "Error: Could not rollup attribute " + str + ". " + exc.Message );
                    if (apiCall == true)
                    {
                        var updateStatus = Builders<RollupModel>.Update
			...
			RollupMessaging.AddMessage("Creating the SECTION_# table");
			try
			{
			...
			{
				RollupMessaging.AddMessage("Exception handled during SECTION_# table creation. " + exc.Message);
                return;
			}
			...
			if( ( m_nCountNumber + m_nCountString ) < columnLimit )
			{
				...
				RollupMessaging.AddMessage("Inserting SEGMENT_ table attributes into segment control");
...
#region Create Section ID for LRS AND SRS
			...
			RollupMessaging.AddMessage("Starting through dynamic segmentation table for geometries.");
						...
						if( drGeometry["ROUTES"].ToString() == dr["ROUTES"].ToString() && drGeometry["DIRECTION"].ToString() == dr["DIRECTION"].ToString() )
						{
							...
							if( !String.IsNullOrEmpty( strGeom ) )
							{
								...
								else
								{
									...
									{
										RollupMessaging.AddMessage( "Error processing LINESTRING segment. " + exc.Message );
			...
			RollupMessaging.AddMessage("Finished Geometries");
      ...
      //      }
			...
			// Just get the envelope from the network definition page (along with all this other stuff).
			RollupMessaging.AddMessage("Getting envelope data from NETWORK_DEFINITION");
			if (DBMgr.NativeConnectionParameters.Provider == "ORACLE")
			{
			...
			while( dr.Read() )
			{
				...
				{
					RollupMessaging.AddMessage("Area for Facility: " + dr["FACILITY"].ToString() + " Section: " + dr["SECTION"].ToString() + " could not be parsed.  Value of 1 used.");
				}
			...
			RollupMessaging.AddMessage("Finished adding SRS sections");
			dr.Close();
			tw.Close();
			////Use batch Loader to load SRS data.
			//dsmelser
			//needed for Oracle
			RollupMessaging.AddMessage("Bulk loading SECTION_ table.");
			...
			RollupMessaging.AddMessage("Finished Bulk Loading SECTION_ table.");
			...
			foreach( String strSegmentTable in m_listSegmentTables )
			{
				...
				{
					RollupMessaging.AddMessage( "Rolling up Linear Attribute:" + attribute + " at " + DateTime.Now.ToString( "HH:mm:ss" ) );
					bRollupError = false;
					...
					{
						RollupMessaging.AddMessage("Error rolling up attribute: " + attribute + " - " + exc.Message);
					}
					...
					{
						RollupMessaging.AddMessage( "Warning: Could not open attribute table " + attribute + ". " + exc.Message );
                        if (apiCall == true)
                        {
                            var updateStatus = Builders<RollupModel>.Update
				...
				foreach( DataPoint dp in listSections ) // For each section
				{
					...
					{
						RollupMessaging.AddMessage("Error in RunMethod. " + exc.Message);
					}
				}
				...
				RollupMessaging.AddMessage( strMessage );
				...
				strOutFile = strMyDocumentsFolder + "\\" + strSegmentTable + ".txt";
				RollupMessaging.AddMessage( strMessageLRSBulk );
				...
				if( listSections.Count > 0 )
				{
					switch( DBMgr.NativeConnectionParameters.Provider )
					{
						...
						case "ORACLE":
							...
							{
								RollupMessaging.AddMessage( "Error with Oracle bulk load of segment table: " + ex.Message );
							}
							break;
						default:
							throw new NotImplementedException( "TODO: Create ANSI implementation for XXXXXXXXXXXX" );
				...
				foreach( String attribute in listAttributeInTable )
				{
					RollupMessaging.AddMessage( "Rolling up Section Attribute:" + attribute + " at " + DateTime.Now.ToString( "HH:mm:ss" ) );
					...
					{
						RollupMessaging.AddMessage( "Warning: Could not open SRS attribute table " + attribute + ". " + exc.Message );
                        if(apiCall == true)
                        {
                            var updateStatus = Builders<RollupModel>.Update
				...
				strOutFile = strMyDocumentsFolder + "\\SRS_" + strSegmentTable + ".txt";
				RollupMessaging.AddMessage( strMessageSRSBulk );
			...
			RollupMessaging.AddMessage( "End rollup of network: " + strNetwork + " at " + DateTime.Now.ToString( "HH:mm:ss" ) );
        ...
        private void CreateCalculatedAssets()
        {
            ...
            foreach (String strTable in listTables)
            {
                ...
                {
                    RollupMessaging.AddMessage("Error: Creating Calculated Asset table " + strCalculatedTable +". " + exception.Message);
                    continue;
                }
        ...
        private void CleanCommittedOnSectionNumberChange(String strNetworkID)
        {
            ...
            if (nNumberSection != m_nPreviousNumberSections)
            {
                ...
                {
                    RollupMessaging.AddMessage("Error: Dropping Committed Projects on number of Section change.");
                }
            }
        }
                    ...
                    if (bAllInputsAvailable)
                    {
						try
						{
							...
							if (bCriteriaMet)
							{
								if (strOutputValue != null)
								{
									RollupMessaging.AddMessage("Warning: Multiple valid criteria for " + attribute.m_strAttribute + " section: " + values[0].ToString() + ". " + ac.Criteria.Replace("|", "'") + " discarded.");
								}
						...
						{
							RollupMessaging.AddMessage("Error with RunMethod." + exc.Message);
						}
                    }
                }
            }
...
//                    if (nCompare > 0) //Route is greater than the one we ar looking for.
            ...
            switch (attribute.m_strRollupType)
            {
                ...
                case "Average": //Average
                    ...
                    foreach (DataPoint pt in list)
                    {
						...
						{
							RollupMessaging.AddMessage("Error: " + exc.Message + ". At " + pt.m_strRoutes + ", " + pt.m_strSection + ", " + pt.m_strBegin + ", " + pt.m_strEnd);
						}
                    }
                    if (fCount > 0)
                    {
        ...
        private bool CompileAreaEquation()
        {
            ...
            {
                RollupMessaging.AddMessage("Error in filling default value table for area calculation." + exception.Message);
                return false;
            }
            ...
            {
                RollupMessaging.AddMessage("Error: Error in compiling AREA function. SQL message - " + exception.Message);
                return false;
            }
            ...
            {
                RollupMessaging.AddMessage("Error: Error in compiling AREA function. SQL message - " + m_crArea.m_listError[0].ToString());
                return false;
            }
            ...
            foreach (String str in m_listArea)
            {
                if (str != "LENGTH" && str != "AREA")
                {
                    ...
                    {
                        RollupMessaging.AddMessage("Error: Default values must be set for all inputs used to calculate area.  Default is not set for attribute:" + str);
                        return false;
                    }
                }
            }
