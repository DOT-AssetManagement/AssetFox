...
{
    public class TargetResultsDAL
    {
        private readonly BridgeCareContext db;
        private readonly CellAddress address;

        public TargetResultsDAL(BridgeCareContext context, CellAddress cell)
        {
            db = context ?? throw new ArgumentNullException(nameof(context));
            address = cell ?? throw new ArgumentNullException(nameof(cell));
        }

        public TargetReportModel GetData(Hashtable yearsIDValues, int[] totalYears, Hashtable idTargets)
        {
            ...
            {
                attributeOrder.Add(attribute.ATTRIBUTE_, attribute.ASCENDING ?? true);
            }
            ...
            var dataForTarget = new TargetReportModel
            {
                Targets = targetTable,
                Address = address
            };
